...
def initialize_session_state() -> None:
    defaults = {
        "retracement_button": False,
        "expansion_button": False,
        "range_button": False,
        "breakout_button": True,
        "use_orb_body": False,
    }
...
def render_sidebar() -> Tuple[pd.DataFrame, Dict[str, str]]:
    with st.sidebar:
        symbol_dict = {
        ...
        symbol = st.selectbox("Choose your Symbol?", symbol_dict.keys())

        session_dict = {
            "New York (9:30 - 16:00 EST)": "ny",
        ...
        orb_duration = st.selectbox("Choose Opening Range Duration", [60, 30])

        file = os.path.join(
            "data", f"{symbol.lower()}_{session_dict.get(session)}_{orb_duration}.csv"
        )
        df = load_data(file)
        st.divider()
    ...
    return df, {
        ...
        "session_key": session_dict.get(session),
        "orb_duration": orb_duration,
    }


def apply_primary_filters(df: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, List[str]]]:
    ...
    with select2:
        if data_filter == "Total Dataset":
            ...
            day = st.selectbox(
                "Select day?",
                np.unique(df.index.weekday),
                format_func=lambda x: day_options.get(x),
            )
    ...
    with col3:
        model_list = list(df.model.dropna().unique()) + [
            "All Models",
            "All Upside Models",
            "All Downside Models",
            "Upside + Expansion",
        ...
        model_filter_selection = st.selectbox(
        ...
        if model_filter_selection == "All Upside Models":
            model_filter = [
                "Strong Uptrend",
                "Medium Uptrend",
                "Weak Uptrend",
            ]
        elif model_filter_selection == "All Downside Models":
            model_filter = [
                "Strong Downtrend",
                "Medium Downtrend",
                "Weak Downtrend",
            ]
        elif model_filter_selection == "Upside + Expansion":
            model_filter = [
                "Strong Uptrend",
                "Medium Uptrend",
                "Weak Uptrend",
        ...
        elif model_filter_selection == "Downside + Expansion":
            model_filter = [
                "Strong Downtrend",
                "Medium Downtrend",
                "Weak Downtrend",
        ...
        elif model_filter_selection == "All Models":
            model_filter = [
                "Strong Downtrend",
                "Medium Downtrend",
                "Weak Downtrend",
            ...
            model_filter = [model_filter_selection]
    ...
    breakout_time = st.multiselect(
        "Breakout time of the day", time_windows, default=time_windows
    )

    df = df[(df.breakout_window.isin(breakout_time)) & (df.model.isin(model_filter))]
...
def render_general_tab(tab, df: pd.DataFrame, orb_side: str, data_points: int) -> None:
    with tab:
        ...
        with col4:
            ...
            st.metric(
                "Price closes outside opening range",
                f"{orb_winning_days:.1%}",
        ...
        with col7:
            st.metric(
                "Median Opening Range Size Multiplier",
...
def render_distribution_tab(tab, df: pd.DataFrame, orb_side: str) -> None:
    with tab:
        use_orb_body = st.toggle(
            "Use candle bodys for OR calculation",
            help="Uses bodys to determine the opening range instead of wicks.",
        )

        df_ret = df[["retracement_window", "max_expansion_time"]].groupby(
            ["retracement_window"]
        ).count()
        df_ret["pct"] = df_ret["max_expansion_time"] / df_ret["max_expansion_time"].sum()
        df_exp = df[["max_retracement_time", "expansion_window"]].groupby(
            ["expansion_window"]
        ).count()
        ...
        st.session_state["use_orb_body"] = bool(use_orb_body)
        ...
        with col4:
            ...
            st.metric(
                "Median retracement before HoS/LoS:",
                value=str(median_retracement),
        ...
        with col5:
            ...
            st.metric(
                "Median time of max expansion:",
                value=str(median_expansion),
        ...
        if st.session_state["breakout_button"]:
            ...
            if orb_side == "Short":
                if not st.session_state["use_orb_body"]:
                    df2 = create_plot_df(
                        df,
                        "retracement_level",
                        inverse_percentile=False,
                        ascending=True,
                    )
                else:
                    df2 = create_plot_df(
                        df,
                        "retracement_level_body",
                        inverse_percentile=False,
                        ascending=True,
                    )
            else:
                if not st.session_state["use_orb_body"]:
                    df2 = create_plot_df(
                        df, "retracement_level", inverse_percentile=True
                    )
                else:
                    df2 = create_plot_df(
                        df, "retracement_level_body", inverse_percentile=True
                    )
            ...
            with tab_chart:
                if orb_side == "Short":
                    fig = create_plotly_plot(
                        df2,
                        "Distribution of max retracement before low of the session",
                        "Retracement Level",
                        reversed_x_axis=False,
                        bar_color=bar_color,
                    ...
                    fig = create_plotly_plot(
                        df2,
                        "Distribution of max retracement before high of the session",
                        "Retracement Level",
                ...
                st.divider()
                use_minutes = st.toggle(
                    "Use minutes",
                    help="Shows the max retracement in minutes after breakout instead of an absolute time value",
                    value=False,
                    key="minute",
                )
                if use_minutes:
                    ret_df = (
                        df.groupby("retracement_in_minutes")
                        .agg({"max_retracement_time": "count"})
                        .reset_index()
                    )
                    ret_df = ret_df.rename(
                        columns={"max_retracement_time": "count"}
                    )
                    ret_df["percentile"] = (
                        ret_df["count"].cumsum() / ret_df["count"].sum()
                    )
                    ret_df = ret_df.set_index("retracement_in_minutes")
                    x_title = "Max Retracement in minutes after breakout"
                else:
                    ret_df = (
                        df.groupby("retracement_window")
                        .agg({"retracement_in_minutes": "count"})
                        .reset_index()
                    )
                    ret_df = ret_df.rename(
                        columns={"retracement_in_minutes": "count"}
                    )
                    ret_df["percentile"] = (
                        ret_df["count"].cumsum() / ret_df["count"].sum()
                    )
            ...
            st.line_chart(
                df_ret[["pct retracement", "pct expansion"]],
                color=[bar_color, line_color],
            )
            ...
            if orb_side == "Short":
                if not st.session_state["use_orb_body"]:
                    df2 = create_plot_df(
                        df,
                        "expansion_level",
                        inverse_percentile=True,
                        ascending=False,
                    )
                else:
                    df2 = create_plot_df(
                        df,
                        "expansion_level_body",
                        inverse_percentile=True,
                        ascending=False,
                    )
            else:
                if not st.session_state["use_orb_body"]:
                    ...
                    df2 = create_plot_df(
                        df, "expansion_level_body", inverse_percentile=False
                    )
            ...
            with tab_chart:
                if orb_side == "Short":
                    fig = create_plotly_plot(
                        df2,
                        "Distribution of max expansion before high/low of the session",
                        "Expansion Level",
                        reversed_x_axis=True,
                        bar_color=bar_color,
                        line_color=line_color,
                ...
                if st.session_state["minute2"]:
                    exp_df = (
                        df.groupby("expansion_in_minutes")
                        .agg({"max_expansion_time": "count"})
                        .reset_index()
                    )
                    exp_df = exp_df.rename(
                        columns={"max_expansion_time": "count"}
                    )
                    exp_df = exp_df.set_index("expansion_in_minutes")
                    x_title = "Max Expansion Time"
                else:
                    exp_df = (
                        df.groupby("expansion_window")
                        .agg({"expansion_in_minutes": "count"})
                        .reset_index()
                    )
                    exp_df = exp_df.rename(
                        columns={"expansion_in_minutes": "count"}
                    )
                    exp_df = exp_df.set_index("expansion_window")
                    x_title = "Max Expansion Time"

                exp_df["percentile"] = (
                    exp_df["count"].cumsum() / exp_df["count"].sum()
                )

                fig3 = create_plotly_plot(
                    df=exp_df,
                ...
                st.write(
                    f"Median max expansion time is: {median_time_calcualtion(df.max_expansion_time)}"
                )
...
def render_model_section(
    tab, df: pd.DataFrame, orb_side: str, greenbox: str, model_filter: List[str]
) -> None:
    with tab:
        ...
        if (len(model_filter) - 1 != len(order)) or (orb_side != "All") or (greenbox != "All"):
            st.error(
                "This section is used to determine the probability of the current session model. "
        ...
        model_df["model_prev"] = pd.Categorical(
            model_df["model_prev"], categories=order, ordered=True
        )
        ...
        with md_up_col:
            is_md_true = st.selectbox(
                "Has previous Session ORB rule hold true?", [True, False]
            )

        scenario_sel = st.multiselect(
            "Potential scenarios for current session",
            order,
            order,
        ...
        if not sankey_bool:
            ...
            all_labels = list(
                pd.concat([df_sankey["model_prev"], df_sankey["model"], df_sankey["upday"]]).unique()
            )
            ...
            df_sankey["prev_model2"] = df_sankey["model_prev"].replace(
                "_prev_session", "", regex=True
            )

            df_sankey = df_sankey[
                (df_sankey["model"].isin(scenario_sel))
            ...
            fig = go.Figure(
                go.Sankey(
                    ...
                    link=dict(
                        source=link_data["source"].to_list()
                        + link_data["target"].to_list(),
                        target=link_data["target"].to_list()
                        + link_data["target2"].to_list(),
                        value=link_data["value"].to_list()
                        + link_data["value"].to_list(),
                    ),
                )
            )
...
def render_strategy_tester(tab, df: pd.DataFrame, orb_side: str) -> None:
    with tab:
        if orb_side == "All":
            st.error(
                "Please select a breakout side! Long breakout assumes long trade and vice versa."
            )
            return
        ...
        with col_buy_in:
            if orb_side == "Short":
                buy_in = round(
                    st.number_input(
                        "What is your sell in level:", step=0.1, value=0.0
                    ),
                    2,
                )
            else:
                buy_in = round(
                    st.number_input(
                        "What is your buy in level:", step=0.1, value=1.0
                    ),
                    2,
                )
        with col_sl:
        ...
        with col_tp:
            if orb_side == "Short":
                tp = st.number_input(
                    "What is your take profit level:", step=0.1, value=-0.5
                )
            else:
                tp = st.number_input(
                    "What is your take profit level:", step=0.1, value=1.5
                )

        if orb_side == "Long":
            strat_df = df[
                (df.retracement_level <= buy_in)
                | (
                    (df.retracement_level > buy_in)
                    & (df.after_conf_min_level <= buy_in)
                )
            ]
            strat_df = strat_df[
                [
                    "after_conf_max_level",
                    "after_conf_min_level",
                    "session_close_level",
                    "retracement_level",
                    "expansion_level",
                ]
            ...
            tp_df = strat_df[
                (strat_df.expansion_level >= tp) & (strat_df.retracement_level > sl)
            ]
            ...
            strat_df = df[
                ...
                | (
                    (df.retracement_level < buy_in)
                    & (df.after_conf_max_level > buy_in)
                )
            ]
            strat_df = strat_df[
                [
        ...
        win_rate = (part_win_count + tp_count) / trade_count if trade_count else 0
        risk_unit = abs(buy_in - sl)
        target_tp = abs(buy_in - tp) / risk_unit if risk_unit else 0
        ...
        with profit_factor:
            if np.isclose(risk_unit, 0):
                ...
                if orb_side == "Long":
                    part_win_r_values = (
                        (part_win_df.session_close_level - buy_in) / risk_unit
                    )
                    part_loss_r_values = (
                        (buy_in - part_loss_df.session_close_level) / risk_unit
                    )
                else:
                    part_win_r_values = (
                        (buy_in - part_win_df.session_close_level) / risk_unit
                    )
                    part_loss_r_values = (
                        (part_loss_df.session_close_level - buy_in) / risk_unit
                    )
            ...
            if np.isclose(loss_r, 0):
                ...
                st.info(
                    "Profit factor cannot be calculated because total loss R equals zero."
                )
            else:
        ...
        with avg_rr:
            ...
            if np.isclose(denom, 0):
                real_par_rr = np.zeros(len(part_df))
            elif orb_side == "Long":
                real_par_rr = (
                    np.array(part_df.session_close_level) - buy_in
                ) / denom
            else:
                real_par_rr = (
                    np.array(buy_in - part_df.session_close_level)
                ) / denom
            avg_risk_reward = (
                (tp_count * target_tp)
                + (sl_count * -1)
                + sum(real_par_rr)
            ) / trade_count if trade_count else 0
        ...
        st.caption(
            ...
            "Unlike candle-to-candle backtesting methods, which analyze each individual candle's data, this tool utilizes vectorized operations based on specific levels to assess trade success. "
            "While every effort has been made to accurately simulate trading conditions, there are inherent limitations in any backtesting approach. "
            "Factors such as slippage, market volatility, and liquidity conditions are not be fully accounted in this simplified model. "
            "Therefore, it's important to interpret the results of this backtest with caution and consider them as a guide rather than a definitive prediction of actual trading performance. "
        )


def render_ml_section(tab, symbol: str, greenbox: str, session_key: str) -> None:
    with tab:
        st.write(
        ...
        if greenbox == "All":
            st.error(
                "Please select a greenbox status. It´s an important feature of the ML prediction."
            )

        open_level = st.selectbox(
            "What is the price level of the opening price?", [i / 10 for i in range(11)]
        )
        close_level = st.selectbox(
            "What is the price level of the closing price?", [i / 10 for i in range(11)]
        )
        pred_values = [[1 if greenbox == "True" else 0][0], open_level, close_level]

        model, scaler = load_ml_model(symbol, session_key)
        if model == 0:
            st.subheader(scaler)
            return
        ...
        if y_predicted[0] == 0:
            st.subheader(
                "The machine learning model predicts a :red[short] breakout for this session!"
            )
        else:
            st.subheader(
                "The machine learning model predicts a :red[long] breakout for this session!"
            )


def render_strategy_rules(tab) -> None:
    with tab:
        st.subheader("Understanding the Opening Range Strategy")
        st.write(
            "The Opening Range strategy centers around the initial price movements that occur during the first hour of market open. This period, known as the \"opening range\", sets the tone for the trading session. "
            "But why is the open of a trading session so important? The open often establishes the trend and sentiment for the day! More often than not, the open is near the high or low of the day. "
        )
...
def render_faq_tab(tab) -> None:
    with tab:
        ...
        orb_rule.write(
            "No trading recommendation can be derived from this. Please read the disclaimer very carefully."
        )
...
def render_disclaimer(tab) -> None:
    with tab:
        st.write(
...
def main() -> None:
    ...
    df, sidebar_state = render_sidebar()

    symbol = sidebar_state["symbol"]
    symbol_dict = sidebar_state["symbol_dict"]

    st.header("Opening Range Breakout Analytics")
    st.write(f":red[{symbol_dict.get(symbol)} ]")

    df, filter_state = apply_primary_filters(df)
    ...
    render_general_tab(general_tab, df, filter_state["orb_side"], data_points)
    render_distribution_tab(distribution_tab, df, filter_state["orb_side"])
    render_model_section(
        model_tab,
        df,
        filter_state["orb_side"],
        filter_state["greenbox"],
        filter_state["model_filter"],
    )
    render_strategy_tester(strategy_tester_tab, df, filter_state["orb_side"])
    render_strategy_rules(strategy_rules_tab)
    render_faq_tab(faq_tab)
    render_disclaimer(disclaimer_tab)
    render_ml_section(
        ml_tab,
        symbol,
        filter_state["greenbox"],
        sidebar_state["session_key"],
    )
    ...
    st.write(
        f"Statistics based on :red[{len(df)}] data points from :red[{start_date}] to :red[{end_date}]"
    )

    if len(df) < 100:
        st.error(
            "The selected sample size is very small. "
